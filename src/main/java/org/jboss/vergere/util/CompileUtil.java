...
public class CompileUtil {
  ...
  public static Class compileAndLoad(final String sourcePath,
                                     final String packageName,
                                     final String className) throws IOException {
    final String tempDirectory = VergereUtils.getTempDirectory();
  ...
  public static String compileClass(final String sourcePath,
    ...
    try {
       ...
       */
      final int compile = adapter.compile(System.out, errorOutputStream, outputPath,
          new File(sourcePath + File.separator + className + ".java").getAbsolutePath(), classPath);
      if (compile != 0) {
  ...
  public static Set<File> findAllMatching(final String fileName, final File from) {
    final java.util.HashSet<File> matching = new java.util.HashSet<File>();
    _findAllMatching(matching, fileName, from);
    return matching;
  }

  @SuppressWarnings("ConstantConditions")
  public static void _findAllMatching(final java.util.HashSet<File> matching, final String fileName, final File from) {
    if (from.isDirectory()) {
